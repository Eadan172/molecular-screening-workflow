...
=======
    ...
    df_train = pd.concat([X_train.reset_index(drop=True), pd.Series(np.expm1(y_train), name='IC50')], axis=1)
    df_train.to_csv('./results/QSAR/qsar_training_data.csv', index=False)
...
=======
    ...
    pipeline = Pipeline([
        ...
        ('feature_selection', SelectKBest(f_regression, k=min(150, X_train.shape[1]))),
    ...
    print("Training Optimized Voting Ensemble model...")
    ...
    print(f"Root Mean Squared Error (RMSE) on the validation set: {rmse:.4f}")
    print(f"R²目标 (>0.8): {'达成' if r2 > 0.8 else '未达成'}")
    print(f"RMSE目标 (<0.2): {'达成' if rmse < 0.2 else '未达成'}")
    
    # 保存模型性能记录
    performance = {
        'model_type': 'Optimized Voting Ensemble (RF + GB)',
        'architecture': {
        ...
        'n_features_selected': min(150, X_train.shape[1]),
        'n_samples_train': int(len(X_train)),
        'n_samples_val': int(len(X_val)),
        'targets_met': {
            'r2_gt_08': bool(r2 > 0.8),
            'rmse_lt_02': bool(rmse < 0.2)
        },
        'training_date': datetime.now().isoformat()
    }
    os.makedirs(os.path.dirname(model_path), exist_ok=True)
    perf_path = model_path.replace('.pkl', '_performance.json')
